...
# Data upload
Upload the raw data table 

```{r}
data_exp = read.table("data/survival.txt",header=T,dec=".",sep="\t")
```
...
* The data columns are:
    ...
    + **Individual**: code of the measured individual
    + **tot_eaten**: number of tardigrades eaten by that snail (by snail ID)
    + **day**: day when feces were examined

# Analysis on laboratory experiment data
    
```{r}
data_exp$day = factor(data_exp$day, levels = c("d1","d2","d3","d4"))
...
contrasts(data_exp$day) = my_contrasts

# GLMM with snail ID as random effect
mod1 = glmer(cbind(alive_n,tot_eaten-alive_n) ~ day + (1|Snail.ID) , family ="binomial",data = data_exp)
...
## Laboratory feeding experiment data plot
```{r}
